...
def _load_modules():
    ...
    from leaf.scanner.modules import (
...
def require_authorization(target: str, flag_confirmed: bool = False) -> bool:
    ...
    from leaf.ui.tui import console, HAS_RICH, warn

    if HAS_RICH and console:
        console.print()
        console.print("[bold red]⚠  AUTHORIZATION REQUIRED[/bold red]")
        console.print("[dim]Leaf only performs authorized security testing.[/dim]")
        console.print()
...
def save_history_entry(target, report_id, finding_count, profile):
    ...
    try:
        import json
        from leaf.core.config import CONFIG_DIR
...
class ScanEngine:
    ...
    def run(self, live_callback=None, module_callback=None) -> list:
        ...
        from leaf.ui.tui import print_module_result
...
def run_scan(target, config, modules=None, authorized=False, verbose=False, live_callback=None, module_callback=None):
    ...
    """
    from leaf.ui.tui import print_scan_start, print_summary, warn
