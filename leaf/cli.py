...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    from leaf.ui.tui import console, HAS_RICH, ASCII_LOGO
    if HAS_RICH:
        console.print(ASCII_LOGO, style="bold green")
        console.print(f"\n  [bold green]Leaf[/bold green] v[bold]{__version__}[/bold]")
        console.print(f"  [dim]By {__author__} · {__company__}[/dim]")
        console.print(f"  [dim]{__github__}[/dim]\n")
    else:
        print(f"Leaf v{__version__} by {__author__} ({__company__})")
    ctx.exit()
...
def main(ctx):
    """
    🌿 Leaf v3.0.0 — Collaborative AI Scanner & Compliance Auditor
    ...
    \b
    QUICK START:
      leaf setup                         First-run configuration wizard
      leaf scan https://target.com       Scan a target (requires authorization)
      leaf ask what is XSS               Ask questions directly to the AI RAG engine
      leaf code                          Launch interactive Leaf Code AI coding partner
      leaf audit --dir <path>            Run local SAST codebase security audit
      leaf report list                   List saved reports
    """
    if ctx.invoked_subcommand is None:
        # Check if first run
        from leaf.core.config import is_first_run
        if is_first_run():
            click.echo("\n  No config found. Run: leaf setup\n")
        else:
...
def setup():
    ...
    from leaf.core.setup_wizard import run_setup
    try:
        run_setup()
    except KeyboardInterrupt:
        click.echo("\n\n  Setup interrupted. Run 'leaf setup' to try again.\n")
        sys.exit(0)
...
def scan(target, modules, exclude_modules, profile, output, json_only, verbose, authorized, no_save, continuous, submit, min_severity, custom_headers, rate_limit, random_agent, silent, csv_path, html_path):
    ...
    \b
    EXAMPLES:
      leaf scan https://example.com --i-have-permission
      leaf scan https://app.local -m headers,tls,ports -p stealth
      leaf scan https://target.com -m xss,sqli --verbose
    """
    from leaf.core.config import load_config, SCAN_PROFILES
    from leaf.scanner.engine import run_scan, ALL_MODULES
    from leaf.report.generator import save_report, ai_classify_finding
    from leaf.scanner.engine import save_history_entry
    from leaf.ui.tui import print_banner, success, warn, error, info, console, HAS_RICH
    ...
    from leaf.scanner.engine import require_authorization
    ...
    try:
        while True:
            ...
            if cfg.get("ai", {}).get("enabled"):
                from leaf.report.generator import ai_classify_finding
            ...
            if csv_path:
                from leaf.report.exporters import export_to_csv
                export_to_csv(findings, csv_path)
            if html_path:
                from leaf.report.exporters import export_to_html
...
def history(limit):
    ...
    from leaf.scanner.engine import load_history
    from leaf.ui.tui import console, HAS_RICH, print_banner, print_divider

    print_banner("Scan History")

    entries = load_history()
    if not entries:
        click.echo("  No scan history yet. Run: leaf scan https://target.com\n")
        return
...
def report_list(limit):
    """List saved reports."""
    from leaf.report.generator import list_reports
    from leaf.ui.tui import console, HAS_RICH, print_banner

    print_banner("Saved Reports")
    reports = list_reports()

    if not reports:
        click.echo("  No reports saved yet. Run: leaf scan https://target.com\n")
        return
...
@main.command()
def update():
    """Update Leaf to the latest version from PyPI."""
    import subprocess
    from leaf.ui.tui import console, HAS_RICH, print_banner, success, info, warn
    ...
    try:
        import requests
        r = requests.get("https://pypi.org/pypi/leaf/json", timeout=10)
        if r.ok:
            latest = r.json()["info"]["version"]
            info(f"Latest version: {latest}")
            if latest == __version__:
                success("Leaf is already up to date!")
                return
        else:
            latest = None
    except Exception:
    ...
    info("Running: pip install --upgrade leaf")
    try:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "leaf"]
        ...
        if result.returncode == 0:
            success("Leaf updated successfully! Restart your terminal or run 'leaf --version'.")
        else:
            warn(f"Update may have failed:\n{result.stderr}")
    except Exception as e:
        warn(f"Could not run pip: {e}")
        info("Try manually: pip install --upgrade leaf")
...
def auth_login(url):
    ...
    import getpass
    from leaf.core.auth import LeafAPI
    from leaf.core.config import load_config, save_config
    from leaf.ui.tui import prompt, success, warn, error, print_banner
...
def auth_logout():
    ...
    from leaf.core.config import load_config, save_config
    from leaf.ui.tui import success
    cfg = load_config()
    cfg["platform"]["token"]    = ""
...
def auth_status():
    ...
    from leaf.core.auth import LeafAPI
    from leaf.core.config import load_config
    from leaf.ui.tui import console, HAS_RICH, success, warn
    ...
    if token and username:
        ...
        warn("Not authenticated. Run: leaf auth login")
...
def config_cmd(action, key_value):
    ...
    EXAMPLES:
      leaf config show
      leaf config set scan.threads 20
      leaf config set ai.enabled true
      leaf config reset
    """
    from leaf.core.config import load_config, save_config, DEFAULT_CONFIG, CONFIG_FILE
    from leaf.ui.tui import console, HAS_RICH, success, info, warn, error
    ...
    if action == "show":
        ...
        if len(key_value) < 2:
            error("Usage: leaf config set <section.key> <value>")
            sys.exit(1)
...
def help_cmd():
    ...
    from leaf.ui.tui import console, HAS_RICH, print_banner, print_divider, ASCII_LOGO
    ...
    if HAS_RICH and console:
        ...
        console.print(f"""
  [bold white]Leaf CLI v{__version__}[/bold white] — Collaborative AI Security Scanner & SAST Auditor
  
  [bold green]SCANNING & SAST AUDITS[/bold green]
  [green]leaf scan <target>[/green]                    Scan a target URL or host
  [green]leaf scan <target> --min-severity high[/green] Filter report output by severity
  [green]leaf scan <target> --exclude-modules ports[/green] Exclude specific modules
  [green]leaf scan <target> --header "Key:Val"[/green]  Inject custom request headers
  [green]leaf scan <target> --html-export r.html[/green] Save interactive HTML dashboard report
  [green]leaf audit --dir <path>[/green]                 Run local SAST codebase security audit

  [bold green]AI CYBERSECURITY SANDBOX (RAG INTEGRATED)[/bold green]
  [green]leaf ask "<question>"[/green]                  Query Leaf Security AI (hybrid offline/online corporate RAG)
  [green]leaf opencode[/green]                          Launch Leaf Code (interactive AI coding partner)
  [green]leaf chain <report_id>[/green]                  Simulate multi-stage exploit chain from scan
  [green]leaf patch <finding_no> --dir <path>[/green]   Generate Git unified diff patch for finding

  [bold green]COMPLIANCE & RAG DATA[/bold green]
  [green]leaf skills setup[/green]                 Download and install 817 skills library
  [green]leaf skills list[/green]                  List imported compliance skills & mappings
  ...
  * Local dataset RAG matches from 500MB corporate security corpus (/home/kali/.gemini/antigravity/scratch/leaf-ai-llm/).
  ...
  [green]leaf history[/green]                          View scan history logs
  [green]leaf report list[/green]                      List saved markdown/json reports
  [green]leaf report show <id>[/green]                 Show report content details

  [bold green]SETUP & AUTH[/bold green]
  [green]leaf setup[/green]                            Interactive setup wizard
  [green]leaf auth login[/green]                       Log in to platform instance
  [green]leaf auth logout[/green]                      Log out
  [green]leaf auth status[/green]                      Show login auth status
  [green]leaf config show[/green]                      Show current configuration
  [green]leaf config set <key> <value>[/green]         Update configuration values
  [green]leaf update[/green]                           Update to latest version
  [green]leaf --version[/green]                        Show version information
  ...
  [dim]• See: https://github.com/Jovinap/leaf/blob/main/RESPONSIBLE_USE.md[/dim]
""")
    else:
        from leaf import __version__
        print(f"\n  Leaf CLI v{__version__} — Run 'leaf --help' for command reference.\n")
...
def chain(report_id_or_file):
    ...
    """
    from leaf.ui.tui import console, HAS_RICH, print_banner, info, error
    from leaf.core.config import load_config
    from leaf.core.chain_simulator import simulate_exploit_chain
...
def patch(finding_index_or_id, source_dir, report_id_or_file):
    ...
    from leaf.ui.tui import console, HAS_RICH, print_banner, info, error
    from leaf.core.config import load_config
    from leaf.core.patch_generator import generate_git_patch
...
def audit(source_dir):
    ...
    """
    from leaf.ui.tui import console, HAS_RICH, print_banner, info, error, success
    from leaf.scanner.modules.sast_checks import run_local_sast
...
def _load_findings_helper(report_name_or_id):
    from leaf.report.generator import list_reports
    from leaf.core.config import FINDINGS_DIR
    import json
...
def ask(question):
    ...
    if not question_str:
        error("Please provide a question. Example: leaf ask what is XSS")
        sys.exit(1)
...
def skills_list():
    ...
    if not skills_dir.exists():
        error("No skills loaded. Run 'leaf skills setup' to download the Anthropic Cybersecurity Skills library.")
        return
...
def skills_setup():
    ...
    target_dir = Path("/home/kali/.gemini/antigravity/scratch/leaf/.agents")
...
# ── leaf opencode ──────────────────────────────────────────────────────────
@main.command("code")
@click.option("--dir", "target_dir", default=".", help="Directory to load into Leaf Code.")
def code(target_dir):
    """
    Launch Leaf Code — Integrated AI Coding & Compliance Agent.
    ...
    print_banner("Leaf Code v3.0.0")
    if HAS_RICH and console:
        console.print("[dim]Powered by Leaf Security AI · Jovin Joel Groups of Company[/dim]\n")
        info(f"Initialized Leaf Code workspace: {target_dir}")
        ...
        print(f"Initialized Leaf Code workspace: {target_dir}")
    ...
    while True:
        try:
            query = click.prompt("leaf-code> ", prompt_suffix="")
            if query.strip().lower() in ("exit", "quit"):
                if HAS_RICH and console:
                    success("Goodbye from Leaf Code!")
                else:
                    print("Goodbye from Leaf Code!")
                break
            if not query.strip():
                continue
            ...
            system_prompt = """You are Leaf Code, an active AI coding and security engineering assistant built by Leaf Security AI.
...
Keep your explanations concise. Always use these specific formats for system actions so the Leaf CLI can execute them on your behalf."""
            ...
            if HAS_RICH and console:
                from rich.markdown import Markdown
                console.print("\n[bold green]Leaf Code Response:[/bold green]")
                console.print(Markdown(response))
                console.print()
            else:
                print("\nLeaf Code Response:")
            ...
            for path_str, content in write_matches:
                ...
                if click.confirm(f"🌿 [Leaf Code] Do you want to write file to {resolved_path}?", default=True):
                    try:
            ...
            for cmd in cmd_matches:
                ...
                if not cmd:
                    continue
                if click.confirm(f"🌿 [Leaf Code] Do you want to run command: '{cmd}'?", default=True):
                    try:
                        if HAS_RICH and console:
                        ...
                        if click.confirm("Send command execution result back to Leaf Code?", default=True):
                            if HAS_RICH and console:
                                info("Thinking...")
                            else:
                                print("Thinking...")
