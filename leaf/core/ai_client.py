...
class AIClient:
    ...
    def call_ai(self, prompt: str, system: str = "") -> str:
        ...
        if self.provider != "leaf-ai" and not self.api_key:
            return "⚠️ AI integration is configured but API key is missing. Run 'leaf setup' to add."
        ...
        try:
            ...
            if "openrouter.ai" in self.api_url:
                headers["HTTP-Referer"] = "https://github.com/Jovinap/leaf"
                headers["X-Title"] = "Leaf v2"
    ...
    def _call_leaf_ai(self, prompt: str, system: str) -> str:
        ...
        if is_offline:
            ...
            if response_parts:
                ...
                return "[Leaf Security AI - Offline Mode]\nOffline mode active but no matching local corporate dataset context or cybersecurity skills files were found. Run 'leaf skills setup' to download the skills database or configure an API key for online queries."
        ...
        if openrouter_key:
            try:
                headers = {
                    ...
                    "Content-Type": "application/json",
                    "HTTP-Referer": "https://github.com/Jovinap/leaf",
