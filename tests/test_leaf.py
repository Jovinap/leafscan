...
class TestVersion:
    def test_version_format(self):
        import leaf
        parts = leaf.__version__.split(".")
        assert len(parts) == 3
        assert all(p.isdigit() for p in parts)

    def test_metadata(self):
        import leaf
        assert leaf.__author__ == "A.P.Jovin"
        assert "JJ Groups" in leaf.__company__
        assert "Leaf Security AI" in leaf.__product__
...
class TestScanEngine:
    def test_summarize_empty(self):
        from leaf.scanner.engine import summarize
        s = summarize([])
    ...
    def test_all_modules_registered(self):
        from leaf.scanner.engine import ALL_MODULES
...
class TestReportGenerator:
    def test_report_id_format(self):
        from leaf.report.generator import generate_report_id
        rid = generate_report_id()
        assert rid.startswith("LS-")
        assert len(rid) == len("LS-20241215-143022")

    def test_generate_markdown_report(self):
        from leaf.report.generator import generate_markdown_report
        findings = [
            {
                "title": "Test Finding",
                "severity": "high",
...
class TestModules:
    ...
    def test_all_modules_importable(self):
        modules = [
            "leaf.scanner.modules.port_scan",
            "leaf.scanner.modules.header_scan",
            "leaf.scanner.modules.tls_scan",
            "leaf.scanner.modules.dns_scan",
            "leaf.scanner.modules.dir_scan",
            "leaf.scanner.modules.xss_probe",
            "leaf.scanner.modules.sqli_probe",
            "leaf.scanner.modules.cve_patterns",
            "leaf.scanner.modules.info_disclosure",
            "leaf.scanner.modules.misconfig",
        ]
        for m in modules:
    ...
    def test_module_run_returns_list(self):
        ...
        from leaf.scanner.modules import header_scan
        from leaf.core.config import DEFAULT_CONFIG
...
class TestV215Features:
    ...
    def test_json_schema_validation(self):
        from leaf.report.exporters import validate_finding_json
        valid = {
            "title": "Test XSS",
            "severity": "high",
            "vuln_type": "XSS",
    ...
    def test_skills_list_command(self):
        from click.testing import CliRunner
        from leaf.cli import skills_list
        runner = CliRunner()
    ...
    def test_code_command(self):
        from click.testing import CliRunner
        from leaf.cli import code
        runner = CliRunner()
        # Simulate typing 'exit' to immediately exit the TUI loop
        result = runner.invoke(code, ["--dir", "/tmp"], input="exit\n")
        assert result.exit_code == 0
        assert "Leaf Code" in result.output
